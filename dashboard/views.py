...
def index(request):
    ...
    # ===== Dashboard 1 - ALL devices (never scoped by device type) =====
    ...
    at_risk_all = analytics.devices_at_risk(hours=hours, site=site)
    ...
    threats_all = sum(p["threats"] for p in trend_all["points"])
    critical_all = next(
        (s["count"] for s in severity_all if s["severity"] == "Critical"), 0
    )
    ...
    t_at_risk = analytics.devices_at_risk(hours=hours, site=site, device_type=selected)

    type_metrics = {
        "devices": t_row["devices"] if t_row else 0,
        "at_risk": len(t_at_risk),
    ...
    context = {
        ...
        "widgets": {
            ...
            "at_risk": len(at_risk_all),
            "quarantined": unauthorized,
            "threats_window": threats_all,
            "critical": critical_all,
        },
        "correlation": corr,
        "leaderboard": leaderboard,
        "at_risk_top": at_risk_all[:10],
